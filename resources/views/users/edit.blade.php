    ...
    <div class="d-flex align-items-center mb-4" style="gap:14px;">
        ...
        <div>
            <h4 style="font-size:1.3rem;font-weight:700;color:#1c1c1e;margin:0 0 2px;">
                <i class="fas fa-user-edit" style="color:var(--ios-blue);margin-right:8px;"></i>
                Edit User
            </h4>
            <p style="color:var(--ios-gray);font-size:.82rem;margin:0;">
                {{ $user->name }} {{ $user->last_name }}
                &nbsp;·&nbsp;
                <span style="color:{{ $user->status==='active' ? '#1a7a35' : ($user->status==='suspended' ? '#c0392b' : '#555') }};">
                    {{ ucfirst($user->status) }}
                </span>
            </p>
        </div>
    ...
    <form action="{{ route('users.update', $user) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="uf-grid">
            ...
            <div class="uf-col-left">
                ...
                <div class="card mb-3">
                    <div class="card-body" style="padding:1.25rem;">
                        ...
                        <div class="uf-avatar-wrap" id="avatarPreviewWrap">
                            @if($user->avatar)
                                <div class="uf-avatar-placeholder" id="avatarPlaceholder" style="display:none;">
                                    <i class="fas fa-user" style="font-size:2rem;opacity:.25;"></i>
                                </div>
                                <img src="{{ asset('storage/'.$user->avatar) }}"
                                     id="avatarPreview" class="uf-avatar-preview" alt="{{ $user->name }}">
                            @else
                                <div class="uf-avatar-placeholder" id="avatarPlaceholder">
                                    <i class="fas fa-user" style="font-size:2rem;opacity:.25;"></i>
                                    <span style="font-size:.72rem;color:var(--ios-gray);margin-top:4px;">No photo</span>
                                </div>
                                <img src="" id="avatarPreview" class="uf-avatar-preview" style="display:none;" alt="">
                            @endif
                        ...
                        @if($user->avatar)
                            <label class="uf-remove-check">
                                <input type="checkbox" name="remove_avatar" value="1">
                                <span>Remove photo</span>
                            </label>
                        @endif
                        <p style="text-align:center;font-size:.72rem;color:var(--ios-gray);margin:6px 0 0;">
                            JPG, PNG — max 2MB
                        </p>
                    </div>
                </div>
                ...
                <div class="card mb-3">
                    <div class="card-body" style="padding:1.25rem;">
                        ...
                        <div class="uf-radio-stack">
                            @foreach(['active'=>['Active','#34C759'],'inactive'=>['Inactive','#8E8E93'],'suspended'=>['Suspended','#FF3B30']] as $val=>[$lbl,$hex])
                                <label class="uf-radio-row" style="{{ $user->id===auth()->id() ? 'opacity:.5;pointer-events:none;' : '' }}">
                                    <input type="radio" name="status" value="{{ $val }}"
                                           {{ old('status',$user->status)===$val ? 'checked' : '' }}
                ...
                <div class="card">
                    <div class="card-body" style="padding:1.25rem;">
                        <p class="uf-section-label"><i class="fas fa-chart-bar"></i> Account Info</p>
                        <div class="uf-stat-grid">
            ...
            <div class="uf-col-right">
                        ...
                        <div class="uf-row-2" style="margin-bottom:0;">
                            ...
                            <div class="uf-field" style="margin-bottom:0;">
                                ...
                                <input type="text" name="phone_extension"
                                       value="{{ old('phone_extension',$user->phone_extension) }}"
                ...
                <div class="card mb-3">
                    <div class="card-body" style="padding:1.25rem;">
                        <p class="uf-section-label"><i class="fas fa-lock"></i> Change Password</p>
                        <p class="uf-hint" style="margin-bottom:12px;">Leave blank to keep current password.</p>
                        <div class="uf-row-2" style="margin-bottom:0;">
                            <div class="uf-field" style="margin-bottom:0;">
                            ...
                            <div class="uf-field" style="margin-bottom:0;">
                                <label class="uf-label">Confirm Password</label>
                                <div class="uf-pw-wrap">
                                    <input type="password" name="password_confirmation" id="pw2"
                                           class="uf-input" placeholder="Repeat password">
                                ...
                                @foreach(['management','clinical','support'] as $type)
                                    @if($deptGroups->has($type))
                                        ...
                                        @foreach($deptGroups[$type] as $dept)
                                            @php $isChecked = in_array($dept->id, old('department_ids', $userDeptIds)); @endphp
                                            <label class="uf-dept-item" id="deptLabel{{ $dept->id }}">
                                                ...
                                                <span class="uf-primary-star {{ $dept->id == old('primary_dept',$primaryDeptId) ? 'visible' : '' }}"
                                                      id="primary{{ $dept->id }}">★</span>
                                            </label>
                                        @endforeach
                                    @endif
                                @endforeach
                            </div>
                            <input type="hidden" name="primary_dept" id="primaryDeptInput"
                                   value="{{ old('primary_dept',$primaryDeptId) }}">
                ...
                <div class="d-flex justify-content-end" style="gap:10px;">
                    ...
                    <button type="submit" class="uf-btn-submit uf-btn-submit-green">
...
{{-- ══ SHARED STYLES (same as create.blade.php) ════════════════ --}}
<style>
.uf-grid { display:grid; grid-template-columns:240px 1fr; gap:16px; align-items:start; }
...
.uf-back-btn { width:38px;height:38px;border-radius:12px;flex-shrink:0;background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.08);display:flex;align-items:center;justify-content:center;color:#555;text-decoration:none;transition:all .2s; }
.uf-back-btn:hover { background:rgba(0,0,0,.1);color:#1c1c1e;text-decoration:none; }
.uf-section-label { font-size:.75rem;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:var(--ios-gray);margin:0 0 14px;display:flex;align-items:center;gap:6px; }
.uf-section-label i { color:var(--ios-blue); }
.uf-alert-error { display:flex;gap:12px;align-items:flex-start;padding:14px 18px;border-radius:var(--radius-md);background:rgba(255,59,48,.08);border:1px solid rgba(255,59,48,.2);color:#c0392b;font-size:.85rem; }
.uf-alert-error i { flex-shrink:0;margin-top:2px; }
.uf-field { margin-bottom:14px; }
.uf-label { display:block;font-size:.75rem;font-weight:600;color:#1c1c1e;margin-bottom:6px;text-transform:uppercase;letter-spacing:.4px; }
...
.uf-input { width:100%;background:rgba(120,120,128,.08);border:1px solid rgba(0,0,0,.09);border-radius:var(--radius-md);padding:10px 14px;font-size:.875rem;color:#1c1c1e;outline:none;transition:all .2s;appearance:none;-webkit-appearance:none; }
.uf-input:focus { background:#fff;border-color:var(--ios-blue);box-shadow:0 0 0 3px rgba(0,122,255,.12); }
...
.uf-pw-eye { position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--ios-gray);cursor:pointer;font-size:.85rem;padding:2px;transition:color .15s; }
.uf-pw-eye:hover { color:var(--ios-blue); }
.uf-avatar-wrap { position:relative;width:110px;height:110px;border-radius:50%;margin:0 auto 10px;border:2px dashed rgba(0,0,0,.15);overflow:hidden;cursor:pointer;transition:border-color .2s; }
.uf-avatar-wrap:hover { border-color:var(--ios-blue); }
.uf-avatar-placeholder { height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center; }
.uf-avatar-preview { width:100%;height:100%;object-fit:cover;display:block; }
.uf-avatar-overlay { position:absolute;inset:0;border-radius:50%;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.2rem;opacity:0;cursor:pointer;margin:0;transition:opacity .2s; }
.uf-avatar-wrap:hover .uf-avatar-overlay { opacity:1; }
.uf-remove-check { display:flex;align-items:center;gap:7px;justify-content:center;font-size:.75rem;color:var(--ios-red);cursor:pointer;margin:6px 0 0; }
.uf-remove-check input { cursor:pointer;accent-color:var(--ios-red); }
.uf-status-note { font-size:.75rem;color:#b45309;background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.25);border-radius:var(--radius-sm);padding:7px 10px;margin-bottom:10px;display:flex;align-items:center;gap:6px; }
.uf-radio-stack { display:flex;flex-direction:column;gap:8px; }
.uf-radio-row { display:flex;align-items:center;gap:10px;padding:9px 13px;border-radius:var(--radius-md);border:1px solid rgba(0,0,0,.08);background:rgba(120,120,128,.06);cursor:pointer;transition:all .15s;user-select:none; }
.uf-radio-row input[type=radio] { display:none; }
.uf-radio-dot { width:16px;height:16px;border-radius:50%;flex-shrink:0;border:2px solid #ccc;position:relative;transition:all .15s; }
.uf-radio-dot::after { content:'';position:absolute;inset:3px;border-radius:50%;background:transparent;transition:all .15s; }
.uf-radio-row:has(input[value=active]:checked)    { background:rgba(52,199,89,.07);border-color:rgba(52,199,89,.3);color:#1a7a35; }
.uf-radio-row:has(input[value=inactive]:checked)  { background:rgba(142,142,147,.07);border-color:rgba(142,142,147,.3);color:#555; }
.uf-radio-row:has(input[value=suspended]:checked) { background:rgba(255,59,48,.07);border-color:rgba(255,59,48,.3);color:#c0392b; }
...
.uf-stat-box { background:rgba(120,120,128,.07);border-radius:var(--radius-sm);padding:10px 12px;text-align:center; }
...
.uf-dept-list { border:1px solid rgba(0,0,0,.09);border-radius:var(--radius-md);max-height:260px;overflow-y:auto;scrollbar-width:thin;scrollbar-color:rgba(0,0,0,.1) transparent; }
.uf-dept-group { font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.6px;padding:9px 14px 4px;background:rgba(0,0,0,.02);border-bottom:1px solid rgba(0,0,0,.05);position:sticky;top:0; }
.uf-dept-item { display:flex;align-items:center;gap:9px;padding:8px 14px;cursor:pointer;margin:0;font-size:.83rem;color:#555;transition:background .12s;border-bottom:1px solid rgba(0,0,0,.04); }
...
.uf-dept-check { width:18px;height:18px;border-radius:6px;flex-shrink:0;border:1.5px solid rgba(0,0,0,.2);display:flex;align-items:center;justify-content:center;font-size:.6rem;color:transparent;transition:all .15s;background:#fff; }
.uf-dept-item:has(input:checked) .uf-dept-check { background:var(--ios-blue);border-color:var(--ios-blue);color:#fff; }
.uf-dept-name { flex:1;line-height:1.3; }
.uf-primary-star { font-size:.85rem;color:transparent;flex-shrink:0;transition:color .15s;line-height:1; }
.uf-primary-star.visible { color:#FF9500; }
.uf-btn-cancel { display:inline-flex;align-items:center;gap:7px;padding:10px 22px;border-radius:var(--radius-pill);background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.08);color:#555;font-size:.875rem;font-weight:500;text-decoration:none;cursor:pointer;transition:all .2s; }
.uf-btn-cancel:hover { background:rgba(0,0,0,.1);color:#1c1c1e;text-decoration:none; }
.uf-btn-submit { display:inline-flex;align-items:center;gap:8px;padding:10px 28px;border-radius:var(--radius-pill);background:var(--ios-blue);border:none;color:#fff;font-size:.875rem;font-weight:600;cursor:pointer;transition:all .2s var(--ease-ios); }
.uf-btn-submit:hover { background:#0062cc;transform:translateY(-1px);box-shadow:0 6px 20px rgba(0,122,255,.35); }
.uf-btn-submit-green { background:var(--ios-green); }
.uf-btn-submit-green:hover { background:#28a745;box-shadow:0 6px 20px rgba(52,199,89,.35); }
</style>
