...
class FolderController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $user   = $request->user();
        $parent = isset($data['parent_id']) ? Folder::find($data['parent_id']) : null;

        if ($parent) {
            abort_unless(
                PermissionHelper::canOnFolder($user, $parent, 'create')
                || PermissionHelper::canOnFolder($user, $parent, 'create_in_assigned'),
                403, 'You cannot create subfolders here.'
            );
        } else {
            abort_unless(
                PermissionHelper::canOnFolder($user, new Folder, 'create'),
                403, 'You cannot create root folders.'
            );
        }

        $folder = $this->folderService->create($data, $user->id);
