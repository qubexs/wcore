...
class FileService
{
    ...
    public function upload(
        ...
        return DB::transaction(function () use ($uploadedFile, $userId, $folderId, $meta, $extension, $storedPath) {
            // Use Laravel's stream upload for better memory efficiency
            Storage::disk('public')->writeStream($storedPath, fopen($uploadedFile->getRealPath(), 'r'));
    ...
    {
        $files = File::when($folderId, fn ($q) => $q->where('folder_id', $folderId))
